...
def main():
    ...
    for cycle_idx in range(args.training_cycle):
        ...
        try:
            tmp_c_n, tmp_c_r = next(diter)
        except StopIteration:
            diter = iter(dl)
            tmp_c_n, tmp_c_r = next(diter)
        
        tmp_c_n = tmp_c_n.type(torch.FloatTensor)
        tmp_c_n = torch.squeeze(tmp_c_n).to(device)
        tmp_c_n = tmp_c_n.to(device)
        traced_model = torch.jit.trace(model, tmp_c_n)

        traced_model.save(os.path.join(args.model_save_dir, "traced_model.pt"))
